...
class CalendarDataManager{ // 6*7배열에 나타낼 달력 값을 구하는 class
	...
	public Date convertCalToDate(Calendar cal) {
	...
	private void makeCalData(Calendar cal){
		...
		int calStartingPos = 
				(cal.get(Calendar.DAY_OF_WEEK)
						+7-(cal.get(Calendar.DAY_OF_MONTH))%7)%7;
		if(calMonth == 1) calLastDate = 
				calLastDateOfMonth[calMonth] 
				+ leapCheck(calYear);
